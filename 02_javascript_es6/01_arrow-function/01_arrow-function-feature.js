...
let theater = {
    ...
    showMovieList() {
        ...
        /* forEach: Array에서 제공하는 메소드로 배열의 요소별로 돌아가며 콜백 함수를 실행하는 함수 */
        this.titles.forEach (                   // this.titles -> ["파묘", "Dune2", "웡카", "가여운 것들", "포켓몬스터"]
            title => console.log(this.store + ": " + title)         // 화살표 함수는 호출한것보다 상위에 있는 this를 가르킨다.
        );

        // this.titles.forEach(
        //     function (title) {
        //         console.log(this.store + ": " + title);      // this.store가 undefined로 출력된다.
        //     }
        // )
    }
}
...
/* 2. 화살표 함수는 new와 함께 호출할 수 없다.(객체 생성이 되지 않기 때문에) */
...
/*
  ...
  의도적으로 설계된 것이라 할 수 있다.(argument도 마찬가지)
